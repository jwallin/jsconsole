...
server.get('/remote/:id/raw', (req, res) => {
  ...
  sessions.raw[id] = res;
  sessions.raw[id].xhr = req.headers['x-requested-with'] === 'XMLHttpRequest';
});
...
function formatRawLog(dataString) {
  var data;
  try {
  ...
  var type = data.cmd || 'flog';
  type = type.split('.').pop(); // Remove unnecessary stuff
...
server.post('/remote/:id/log', (req, res) => {
  ...
  if (sessions.raw[id]) {
    sessions.raw[id].write(formatRawLog(req.body.data));

    if (sessions.raw[id].xhr) {
      sessions.raw[id].end(); // lets older browsers finish their xhr request
    }
  }
